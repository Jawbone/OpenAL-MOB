...
static void alc_initconfig(void)
{
...
#endif
    if(MOB_ConfigValueStr(root, disable_cpu_exts, &str))
    {
    ...
    RTPrioLevel = 0;
#endif
    MOB_ConfigValueInt(root, rt_prio, &RTPrioLevel);

    if(MOB_ConfigValueStr(root, resampler, &str))
    {
    ...
    else
    {
        ...
        if(str && (strcasecmp(str, "true") == 0 || strtol(str, NULL, 0) == 1))
            TrapALError = AL_TRUE;
        TrapALError = MOB_GetConfigValueBool(root, trap_al_error, TrapALError);

        str = getenv("ALSOFT_TRAP_ALC_ERROR");
        if(str && (strcasecmp(str, "true") == 0 || strtol(str, NULL, 0) == 1))
            TrapALCError = ALC_TRUE;
        TrapALCError = MOB_GetConfigValueBool(root, trap_alc_error, TrapALCError);
    }

    if(MOB_ConfigValueFloat(reverb, boost, &valf))
    ...
    if((str && str[0]) || MOB_ConfigValueStr(root, default_reverb, &str))
...
static ALCenum UpdateDeviceParams(ALCdevice *device, const ALCint *attrList)
{
    ...
    if(device->Type == Loopback)
    {
        ...
        MOB_ConfigValueUInt(root, sends, &numSends);
    ...
    else if(attrList && attrList[0])
    {
        ...
        MOB_ConfigValueUInt(root, frequency, &freq);
	...
	device->ex.HrtfEnabled = AL_FALSE;
    device->Hrtf = NULL;
    if(device->Type != Loopback && MOB_GetConfigValueBool( root, hrtf, AL_FALSE))
	{
        device->Hrtf = GetHrtf(device);
		if( device->Hrtf )
		{
			device->ex.HrtfEnabled = AL_TRUE;
		}
	}
    ...
    if(device->Type != Loopback && !device->Hrtf && MOB_GetConfigValueBool(root, wide_stereo, AL_FALSE))
        device->Flags |= DEVICE_WIDE_STEREO;
...
ALC_API ALCdevice* ALC_APIENTRY alcOpenDevice(const ALCchar *deviceName)
{
    ...
    if(MOB_ConfigValueStr(root, channels, &fmt))
    {
        static const struct {
            const char name[16];
    ...
    if(MOB_ConfigValueStr(root, sample_type, &fmt))
    {
        static const struct {
            const char name[16];
            enum DevFmtType type;
...
#undef DEVICE_FORMAT_REQUEST
    ...
    if(MOB_ConfigValueUInt(root, frequency, &device->Frequency))
    {
        device->Flags |= DEVICE_FREQUENCY_REQUEST;
    ...
    MOB_ConfigValueUInt(root, periods, &device->NumUpdates);
    device->NumUpdates = clampu(device->NumUpdates, 2, 16);

    MOB_ConfigValueUInt(root, period_size, &device->UpdateSize);
    device->UpdateSize = clampu(device->UpdateSize, 64, 8192);
    if((CPUCapFlags&CPU_CAP_SSE))
        device->UpdateSize = (device->UpdateSize+3)&~3;

    MOB_ConfigValueUInt(root, sources, &device->MaxNoOfSources);
    if(device->MaxNoOfSources == 0) device->MaxNoOfSources = 256;

    MOB_ConfigValueUInt(root, slots, &device->AuxiliaryEffectSlotMax);
    if(device->AuxiliaryEffectSlotMax == 0) device->AuxiliaryEffectSlotMax = 4;

    MOB_ConfigValueUInt(root, sends, &device->NumAuxSends);
    if(device->NumAuxSends > MAX_SENDS) device->NumAuxSends = MAX_SENDS;

    MOB_ConfigValueInt(root, cf_level, &device->Bs2bLevel);
